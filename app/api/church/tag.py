...
def base_check(form):
    model = Tag.query.filter(Tag.id == form.id.data).first()
    if not model:
...
def get():
    ...
    data = Tag.query.filter(Tag.name.like(query), Tag.status == 1) \
        .order_by(desc(Tag.create_time)).paginate(page=form.page.data, per_page=form.page_size.data, error_out=False)
    return jsonify(data)
